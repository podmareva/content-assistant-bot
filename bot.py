...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === Контент-Стратег ===
        ...
        if step == 1:
            ...
            try:
                prompt = (
                    f"Ты — контент-стратег. Твоя задача — создать стратегию продвижения для клиента, основываясь на его вводных.\n\n"
                    f"📌 Данные клиента:\n"
                    f"- Распаковка личности и экспертности: {session['data'].get('info', [''])[0]}\n"
                    f"- Позиционирование: {session['data'].get('info', ['',''])[1]}\n"
                    f"- Продукт/услуга: {session['data'].get('info', ['','',''])[2]}\n"
                    f"- Анализ ЦА: {session['data'].get('info', ['','','',''])[3]}\n\n"
                    ...
                    "❗ ВАЖНО: пиши стратегию так, как будто она составлена для клиента (используй его продукт, аудиторию, стиль). "
                    "Не используй от первого лица ('я'), говори от имени клиента. "
                    "Сделай стратегию максимально практичной и применимой.\n\n"
                    "Выдай:\n- План по дням\n- Рубрикатор\n- Привязку к воронке (холодная/тёплая/горячая)\n- Примеры заголовков и CTA (от имени клиента)."
                )
    ...
    # === Планировщик ===
        ...
        if step == 1:
            ...
            try:
                prompt = (
                    f"Ты — контент-планировщик. Составь персональный контент-план для клиента, основываясь на его вводных данных.\n\n"
                    f"📌 Данные клиента:\n"
                    f"- Распаковка личности: {session['data'].get('info', [''])[0]}\n"
                    f"- Позиционирование: {session['data'].get('info', ['',''])[1]}\n"
                    f"- Продукт/услуга: {session['data'].get('info', ['','',''])[2]}\n"
                    f"- Целевая аудитория: {session['data'].get('info', ['','','',''])[3]}\n\n"
                    f"🎯 Параметры плана:\n"
                    f"- Основная соцсеть: {platform}\n"
                    f"- Цель: {goal}\n"
                    f"- Адаптация под другие сети: {adaptation}\n"
                    f"- Частота публикаций: {frequency}\n"
                    f"- От чьего лица: {persona}\n"
                    f"- Срок: {duration} дней\n\n"
                    "❗ ВАЖНО: используй только информацию клиента, пиши так, чтобы это звучало от его имени, а не от тебя. "
                    "Строй темы и идеи, исходя из его экспертизы, продукта и аудитории.\n\n"
                    "Выдай:\n- План на указанный срок\n- Темы с заголовками и CTA\n- Идеи сторис (от клиента)\n- Визуальные подсказки."
                )
                response = openai.ChatCompletion.create(model="gpt-3.5-turbo",
                                                        messages=[{"role": "user", "content": prompt}])
                result = response["choices"][0]["message"]["content"]
    ...
    # === Копирайтер (с поддержкой пост-карусели) ===
        ...
        if step == 2:
            ...
            try:
                prompt = (
                    f"Ты — копирайтер, работающий на клиента. Создай {task}, используя вводные клиента и его стиль.\n\n"
                    f"📌 Данные клиента:\n"
                    ...
                    f"🎯 Задача: {goal}\n📌 Тема: {topic}\n🎨 Тональность: {tone}\nФормат: {format_text}\n\n"
                    f"{format_instruction}\n❗ Пиши так, будто это текст клиента, от его имени. "
                    "Используй его стиль, экспертизу и учитывай его продукт. "
                    "Следи за законом о рекламе РФ, избегай необоснованных обещаний."
                )
                response = openai.ChatCompletion.create(model="gpt-3.5-turbo",
                                                        messages=[{"role": "user", "content": prompt}])
                result = response["choices"][0]["message"]["content"]
    ...
    # === Продюсер Reels ===
        ...
        if step == 1:
            ...
            try:
                prompt = (
                    f"Ты — продюсер Reels. Создай сценарий короткого видео для клиента, строго исходя из его данных.\n\n"
                    f"📌 Данные клиента:\n"
                    f"- Распаковка: {session['data'].get('info', [''])[0]}\n"
                    f"- Позиционирование: {session['data'].get('info', ['',''])[1]}\n"
                    f"- Продукт/услуга: {session['data'].get('info', ['','',''])[2]}\n"
                    ...
                    "❗ Пиши сценарий от лица клиента, учитывай его экспертность, продукт и аудиторию. "
                    "Выдай:\n- Хук (2–3 сек)\n- Основной блок (действия, текст, визуал)\n- CTA\n"
                    "- Альтернативу (с лицом и без)\n- Подсказки по монтажу, свету.\n"
                    "Учитывай тренды 2024–2025 и закон о рекламе."
                )
                response = openai.ChatCompletion.create(model="gpt-3.5-turbo",
                                                        messages=[{"role": "user", "content": prompt}])
                result = response["choices"][0]["message"]["content"]
